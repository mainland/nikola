...
--
-- 'Storable'-based CUDA vectors.
--

module Data.Vector.CUDA.Storable (
  -- * Storable vectors
...
import           Data.Vector.CUDA.Storable.Mutable ( MVector(..) )
import qualified Data.Vector.CUDA.Storable.Mutable as M
...
instance (Data a, Storable a) => Data (Vector a) where
  ...
  dataTypeOf _ = G.mkType "Data.Vector.CUDA.Storable.Vector"
...
instance Storable a => G.Vector Vector a where
  ...
  basicUnsafeSlice i n (Vector _ fp) = Vector n (updPtr (`advanceDevPtr` i) fp)
...
-- Conversions - Host vectors
-- -----------------------------
fromHostVector :: Storable a => S.Vector a -> Vector a
fromHostVector v = unsafePerformIO $ do
    let n  =  S.length v
    mv     <- GM.new n
    S.unsafeWith v $ \ptr  ->
...
toHostVector :: Storable a => Vector a -> S.Vector a
toHostVector v = unsafePerformIO $ do
    let n  =  length v
    mv     <- GM.new n
    unsafeWith v $ \dptr  ->
...
-- | /O(1)/ Create a vector from a 'ForeignDevicePtr' with an offset and a length.
--
-- The data may not be modified through the 'ForeignDevicePtr' afterwards.
--
...
-- | /O(1)/ Create a vector from a 'ForeignDevicePtr' and a length.
--
-- It is assumed the pointer points directly to the data (no offset).
-- Use `unsafeFromForeignDevPtr` if you need to specify an offset.
--
-- The data may not be modified through the 'ForeignDevicePtr' afterwards.
...
unsafeFromForeignDevPtr0 fp n = Vector n fp

-- | /O(1)/ Yield the underlying 'ForeignDevicePtr' together with the offset to the
-- data and its length. The data may not be modified through the 'ForeignDevicePtr'.
unsafeToForeignDevPtr :: Storable a => Vector a -> (ForeignDevicePtr a, Int, Int)
{-# INLINE unsafeToForeignDevPtr #-}
unsafeToForeignDevPtr (Vector n fp) = (fp, 0, n)

-- | /O(1)/ Yield the underlying 'ForeignDevicePtr' together with its length.
--
-- You can assume the pointer points directly to the data (no offset).
--
-- The data may not be modified through the 'ForeignDevicePtr'.
unsafeToForeignDevPtr0 :: Storable a => Vector a -> (ForeignDevicePtr a, Int)
{-# INLINE unsafeToForeignDevPtr0 #-}
unsafeToForeignDevPtr0 (Vector n fp) = (fp, n)

-- | Pass a pointer to the vector's data to the IO action. The data may not be
-- modified through the 'Ptr.
unsafeWith :: Storable a => Vector a -> (DevicePtr a -> IO b) -> IO b
{-# INLINE unsafeWith #-}
unsafeWith (Vector _ fp) = withForeignDevPtr fp
